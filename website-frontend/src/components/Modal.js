...
class Modal extends Component {
 ...
 render(){
    if (this.props.isShow==true){
        ...
        let image = imageCollection.find(imageCollection => imageCollection.id == this.props.modalImgID);
    return(
        ...
        <div className=" p-6 py-6 bg-gray-700 opacity-90 p-2   w-screen h-screen ">    </div>
         <div className="absolute bg-white top-8 w-2/3 h-5/6 border-8 border-opacity-25 border-yellow-200 rounded-lg ">

        <h1 class="viewImageTitle text-3xl text-gray-700 py-6 text-center">{image.title}</h1>

        <img class="viewImage py-6 mx-auto w-auto h-3/5" src={image.url} alt=""/>
        <h1 class="viewImageDescription tex-xl py-4 text-center" >{image.description} </h1>
        <p class="viewImageDate text-xl py-4 text-center">Created at: {image.date}</p>
