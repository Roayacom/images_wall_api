...
this.openModal =imgID =>{
    ...
    const articles = this.props.photoData.map((article) => {
        ...
        return (
            ...
            <article  key={article.id} className="border-2 border-yellow-50 overflow-hidden rounded-lg shadow-lg">
                <img className="w-auto h-56 mx-auto object-cover " src={article.url}/>
                <header className="flex bg-yellow-50 items-center justify-between  md:p-4">
                 
                <h1 className="text-lg font-bold  text-fray-700"> {article.title}</h1>
                <Hyperlink onClick={()=>this.openModal(article.id)}>View</Hyperlink>
